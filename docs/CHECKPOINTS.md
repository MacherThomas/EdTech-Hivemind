...
| 3 | **Valid email domains.** Seeded as `ie.edu`, `student.ie.edu`, `alumni.ie.edu`, which are placeholders to confirm. Should alumni/staff be allowed at all? | IE IT | `SchoolDomain` table via `prisma/seed.ts` |
