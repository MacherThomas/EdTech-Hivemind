...
| 4 | **Sending student content to an AI provider.** AI features are currently **off**. Before setting `AI_ENABLED=true`: an API key from IE's Anthropic org, a DPIA / DPO sign-off, a data processing agreement, and a decision on retention/region. | IE DPO / legal / IT | `src/lib/features.ts`, `src/lib/ai/anthropic.ts` |
